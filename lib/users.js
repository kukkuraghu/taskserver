...
function register(userName, email, password) {
    ...
    function addUser() {
        ...
        getUserbyFbUserEmailPromise.then(fbUserExists, newUser);
...
function fbAuthenticate(fbUserId, fbUserName, fbEmail, fbProfilePicture) {
	...
	function furtherProcessing(user) {
		if (user) {
            ...
            if (user.fbUserId !== fbUserId || user.fbUserName !== fbUserName || user.fbEmail !== fbEmail || user.fbProfilePicture !== fbProfilePicture) {
	...
	function furtherProcessing1(user) {
		if (user) {
			...
			if(user.fbUserName !== fbUserName || user.fbEmail !== fbEmail || user.fbProfilePicture !== fbProfilePicture ) {
...
function linkFbToUser(userId, fbUserId, fbUserName, fbUserEmail, fbProfilePicture) {
	return Users.findByIdAndUpdate(userId, {$set: {fbAccount : {fbUserId: fbUserId, fbUserName :  fbUserName, fbUserEmail : fbUserEmail, fbProfilePicture : fbProfilePicture}}}).exec();
}

function linkUserToFb(userId,  userName, userEmail, userPassword) {
    return Users.findByIdAndUpdate(userId, {$set: {userName: userName, email: userEmail, password: userPassword}}).exec();
}

function updateFbUser(fbUserId, fbUserName, fbUserEmail, fbProfilePicture) {
	return Users.findOneAndUpdate({fbUserId : fbUserId}, {$set: {fbAccount : {fbUserId: fbUserId, fbUserName :  fbUserName, fbUserEmail : fbUserEmail, fbProfilePicture : fbProfilePicture}}}).exec();
}
